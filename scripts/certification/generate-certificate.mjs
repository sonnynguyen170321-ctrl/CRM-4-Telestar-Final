...
`;
  ...
  // Emitted by the generator so it survives regeneration. The certificate is the one document
  // whose currency matters most, and it was the one with no currency marker at all.
  const frontMatter = `---
...
`;
  ...
  writeFileSync(CERTIFICATE_PATH, frontMatter + body);
