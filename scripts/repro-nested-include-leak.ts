...
async function main(): Promise<void> {
  ...
  const rows = await tenantStorage.run({ tenantId: TENANT_A, bypassRls: false }, async () => {
    // The callback must be `async` and must `await` here. Returning the promise from a bare
    // arrow loses the AsyncLocalStorage context before the extension reads it, the store arrives
    // `undefined`, and the query silently takes the *bypass* path — which discloses the foreign
    // relation for a reason that has nothing to do with the defect under test. That mistake is
    // how the first version of this script "reproduced" the bug while proving nothing.
