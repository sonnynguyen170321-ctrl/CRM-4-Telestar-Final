...
describe("scorePoolItem", () => {
  ...
  it("writes an assessment and mirrors it onto the pool record", async () => {
    ...
    const result = await scorePoolItem({
      tenantId: TENANT,
      item,
      icpVersionId,
      rules: RULES,
    ...
    const assessment = await prisma.leadPoolAssessment.findUniqueOrThrow({
  ...
  it("re-scoring the same record under the same rules appends nothing", async () => {
    ...
    const first = await scorePoolItem({
      tenantId: TENANT,
      item,
      icpVersionId,
      rules: RULES,
    });
    const second = await scorePoolItem({
      tenantId: TENANT,
      item,
      icpVersionId,
      rules: RULES,
    ...
    const count = await prisma.leadPoolAssessment.count({
  ...
  it("keeps the old assessment when the rules change, rather than overwriting it", async () => {
    ...
    const tightened = {
      ...RULES,
      industry: { ...RULES.industry, targetIndustries: ["mining"] },
    };
    const second = await scorePoolItem({
      tenantId: TENANT,
      item,
    ...
    expect(
      (assessments[0].rulesSnapshot as any).industry.targetIndustries,
    ).toEqual(["software"]);
    expect(
      (assessments[1].rulesSnapshot as any).industry.targetIndustries,
    ).toEqual(["mining"]);
  ...
  it("changes the fingerprint when either the record or the rules change", () => {
    ...
    const base = assessmentFingerprint(
      buildScoringEvidence(item),
      RULES,
      "icp-v1",
    );

    const otherRecord = assessmentFingerprint(
      buildScoringEvidence({ ...item, title: "Intern" }),
      RULES,
      "icp-v1",
    );
    ...
    expect(
      assessmentFingerprint(buildScoringEvidence(item), RULES, "icp-v2"),
    ).not.toBe(base);
  });
});
