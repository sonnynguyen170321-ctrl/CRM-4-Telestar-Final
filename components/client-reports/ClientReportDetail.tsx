    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-card-border pb-4">
        <div>
          <Link
            href="/client-reports"
            className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-text-primary transition-colors mb-2"
          >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="px-3 py-1.5 bg-card-bg border border-card-border text-text-primary hover:bg-card-border/40 rounded-lg text-xs font-medium transition-colors flex items-center gap-1.5"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3">
        <div className="p-4 bg-card-bg border border-card-border rounded-xl">
          ...
          <div className="text-2xl font-bold text-text-primary mt-1">{snapshot.kpis.leadsTouched.toLocaleString()}</div>
        ...
        <div className="p-4 bg-card-bg border border-card-border rounded-xl">
          ...
          <div className="text-2xl font-bold text-text-primary mt-1">{snapshot.kpis.meetingsCompleted}</div>
          <div className="text-[11px] text-muted-foreground mt-0.5">
            {snapshot.kpis.noShows} no-shows ({(snapshot.kpis.noShowRate * 100).toFixed(1)}%)
          </div>
        </div>

        <div className="p-4 bg-card-bg border border-card-border rounded-xl col-span-2 sm:col-span-4 lg:col-span-1">
          <div className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">
            Accepted Pipeline
          </div>
      ...
      <div className="bg-card-bg border border-card-border rounded-xl p-5 space-y-4 shadow-sm">
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full bg-bg-main border border-card-border rounded-lg p-3 text-xs text-text-primary placeholder:text-muted-foreground focus:ring-1 focus:ring-brand-red focus:outline-none disabled:opacity-75"
          />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
          {/* Key Wins */}
          <div className="p-3.5 bg-card-border/40/30 border border-card-border rounded-lg space-y-2">
            <div className="flex items-center justify-between">
              ...
              {!isApproved && (
                <button
                  ...
                  className="text-[11px] text-muted-foreground hover:text-text-primary flex items-center gap-1"
                >
              ...
              {keyWins.map((win, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  <input
                    ...
                    className="w-full bg-bg-main border border-card-border rounded px-2.5 py-1 text-xs text-text-primary"
                  />
                  {!isApproved && (
                    <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Outreach by Channel */}
        <div className="bg-card-bg border border-card-border rounded-xl p-5 shadow-sm space-y-3">
          <h2 className="text-sm font-bold text-text-primary uppercase tracking-wider">Outreach by Channel</h2>
              ...
              <tbody className="divide-y divide-border/60">
                {snapshot.channels.map((ch) => (
                  <tr key={ch.channel} className="hover:bg-card-border/40/20">
                    <td className="py-2.5 px-3 font-medium text-text-primary flex items-center gap-2">
          ...
          {snapshot.emailChannelHealth && (
            <div className="mt-4 pt-3 border-t border-card-border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-3 bg-card-border/40/20 rounded-lg">
        ...
        {/* Funnel Conversion */}
        <div className="bg-card-bg border border-card-border rounded-xl p-5 shadow-sm space-y-3">
          <h2 className="text-sm font-bold text-text-primary uppercase tracking-wider">Conversion Funnel</h2>
            ...
            {snapshot.funnel.map((step, idx) => (
              <div key={step.stage} className="space-y-1">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-text-primary">{step.count.toLocaleString()}</span>
                    {idx > 0 && (
                ...
                <div className="w-full bg-card-border/40 rounded-full h-2 overflow-hidden">
                  <div
      ...
      <div className="bg-card-bg border border-card-border rounded-xl p-5 shadow-sm space-y-3">
        ...
        {snapshot.meetings.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-card-border bg-card-border/40/30 text-[11px] text-muted-foreground font-semibold">
                ...
                {snapshot.meetings.map((m) => (
                  <tr key={m.id} className="hover:bg-card-border/40/20">
                    <td className="py-2.5 px-3">
                      <div className="font-semibold text-text-primary">{m.company}</div>
                      {m.contactName && (
              ...
              <tbody className="divide-y divide-border/60">
                {snapshot.opportunities.map((opp) => (
                  <tr key={opp.id} className="hover:bg-card-border/40/20">
                    <td className="py-2.5 px-3">
                      <div className="font-semibold text-text-primary">{opp.company}</div>
      ...
      <div className="bg-card-bg border border-card-border rounded-xl p-5 shadow-sm space-y-3">
        <h2 className="text-sm font-bold text-text-primary uppercase tracking-wider">Representative Performance</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-card-border bg-card-border/40/30 text-[11px] text-muted-foreground font-semibold">
            ...
            <tbody className="divide-y divide-border/60">
              {snapshot.reps.map((rep) => (
                <tr key={rep.repId} className="hover:bg-card-border/40/20">
                  <td className="py-2.5 px-3 font-semibold text-text-primary">{rep.displayName}</td>
                  ...
                  <td className="py-2.5 px-3 font-medium text-text-primary">{rep.replies}</td>
