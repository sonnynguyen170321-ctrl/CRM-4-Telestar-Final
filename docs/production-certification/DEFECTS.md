...
**Last Updated**: 2026-08-26T09:34:23.450Z
...
- **Verification evidence**: `STILL OPEN. Re-measured 2026-08-26 against the live schema: 69 public tables, 0 with rowsecurity, 0 policies, and no migration containing ENABLE ROW LEVEL SECURITY. DB_RLS_ENFORCED appears in no environment file and no compose file, and no deploy path applies supabase/roles.sql or supabase/rls.sql. Production tenant isolation rests on the Prisma extension alone. What changed is the strength of the case, which is now measured rather than argued. DB-level RLS would have contained TEL-P0-013 outright: the defective routes ran on the ordinary application client, and the crm_app policy matches on app.current_tenant_id with no branch consulting app.bypass_rls, so the application role cannot grant itself a bypass. A probe added to verify-rls-app-paths.probe.ts shows a foreign tenant reading the victim lead by id returning null under enforcement, with a control proving the owning tenant still reads its own row. The enablement path is also proven to work end to end, collected by scripts/certification/collect-rls-posture.mjs on 2026-08-26: verify-rls PASS exit 0 (15 checks - cross-tenant read, update, delete and insert all denied, fail-closed with no tenant context); verify-rls-app-paths PASS exit 0 ("All application paths survive DB-level RLS"); verify-rls-enablement PASS exit 0 ("the documented enablement sequence produces a working, isolated system"). verify-rls-live is NOT_RUN - it needs CRM_APP_URL, a connection as the application role against a database with roles.sql applied, which exists only on a deployed database. NOT_RUN is not a pass, and EV-RLS-POSTURE records status BLOCKED_EXTERNAL rather than PASS for that reason. Closing this needs the policies applied to the production database and DB_RLS_ENFORCED turned on, which is an infrastructure change requiring operator authorization and Cloud SQL access this workstation does not have.`
...
- **Fix SHA**: `32418164738`
- **Verification evidence**: `scripts/certification/verify-release-identity.mjs now observes the chain instead of transcribing it: it asks the registry for the image at the claimed digest and reads org.opencontainers.image.revision, asks GitHub for the CI run and reads its headSha, and asks the deployment for /api/health - writing each answer to disk as an artifact before judging it. Four links hold, measured 2026-08-26: candidate 9b2b44c9f0987139e2f48ee21b14ec36e10690a8; CI run 32891125645 headSha 9b2b44c conclusion success; image sha256:99fbfe8229e6f298e3c80c8ba280e235ac9b9e528741fc547f3e73fd7364ff2b carrying revision label 9b2b44c, created 2026-08-25T19:51:32Z; health SHA 9b2b44c. Two links do not hold and the record says so rather than rounding up: whether the running web and worker containers were started from that digest is answerable only on the deployment host, and this workstation has no access to it. Those fields are null, the record status is FAIL and REL-001 stays unverified - which was already true and merely unstated, since the previous record asserted all six links with zero artifacts and a five-millisecond duration. Negative controls both run: the previous release's digest reports "image revision c7bf639 is not the candidate", and a CI run from another commit reports "ran on 6b10d54, not the candidate", each exit 1. check U2 fell from 2 findings to 1. STAYS FIXED_PENDING_VERIFICATION: a chain proven on four links out of six is not a proven chain.`
...
- **Fix SHA**: `bea37a533d3b3c2b8843bc254d303f9d5e4ca895`
...
- **Fix SHA**: `N/A`
- **Verification evidence**: `Three of the affected records are now produced by tools that ran, with timestamps from the process clock and artifacts they wrote. EV-DEPLOYED-STATE: scripts/certification/record-deployed-state.mjs, which does not accept the deployed SHA as an argument but probes for it. EV-RELEASE-IDENTITY: scripts/certification/verify-release-identity.mjs, which reads the image revision label, the CI run headSha and the health endpoint. EV-RLS-POSTURE: scripts/certification/collect-rls-posture.mjs, which runs the four RLS scripts and records their exit codes and output. check V fell from 3 findings to 2. Two remain and are named rather than quietly dropped. EV-BRANCH-PROTECTION's behavioural half means opening a disposable pull request with a deliberately failing check and attempting to merge it; its existing 2790-byte transcript re-hashes correctly, so the record is corroborated even though its timestamps were composed. EV-DR-ROLLBACK cannot be regenerated at all until a rollback is actually performed for this candidate, which is TEL-P0-012 and blocked on deployment access. STAYS OPEN until those two are measured.`
...
- **Fix SHA**: `1826fe392c695e9f58772e49fe516e6e5b7297b2`
...
- **Fix SHA**: `1bf5d0d23afee6277d6fdcfc8fa68983f59ce8e8`
...
- **Fix SHA**: `927abc979a61d578979ca5c76bc708e52a8ae5bc`
- **Verification evidence**: `Fixed in two steps. 927abc9 added an explicit tenantId to all three lookups; ab1496d removed the bypass itself, which was the reason they were reachable. Neither route does anything cross-tenant inside the scope - enrich-lead reads one lead then calls the provider, draft-reply reads one lead twice - so bypassRls:true bought nothing and cost the extension's automatic where:{tenantId}, the entire tenant boundary on a database with no RLS policies. Both are now bypassRls:false with the explicit filter kept, so two independent mechanisms make each lookup tenant-correct. Reproduced before the fix, through the real handlers with only the AI provider mocked: tenant-ai-attacker requesting tenant-ai-victim's lead id got back "Victim Holdings AG is an active player..." from enrich-lead and a drafted email reading "Hi Secret, ... could be a fit for Victim Holdings AG?" from draft-reply - 2 failed, 1 passed. Mechanism confirmed outside the harness: victim scoped findFirst -> "Victim Holdings AG"; attacker bypass findUnique -> "Victim Holdings AG"; attacker bypass with explicit tenant filter -> null. After both commits: 3 passed, 0 failed, the third being a control through the same route so a fix that merely broke the lookup could not satisfy it. The second commit also removed a latent break: under DB-level RLS a bypassRls model read returns nothing to anybody, including the owning tenant, because that path sets app.bypass_rls while the crm_app policy matches on app.current_tenant_id, which it never sets. verify-rls-app-paths reported "1 application path(s) would BREAK under DB-level RLS" against the 927abc9 form and "All application paths survive DB-level RLS" after ab1496d. Regression: ai-route-cross-tenant-red-team, object-auth-red-team, access-control, rls, raw-sql-tenant-context, rls-policy-coverage - 6 files, 39 passed, 0 failed. eslint and tsc --noEmit exit 0. Blast radius: 30 prisma calls under a bypassRls:true scope with no tenantId in WHERE were each read; the remainder are system-context cron sweeps, pre-tenant auth lookups, and BullMQ jobRun lookups by dedupeKey, all now recorded in the generated bypass inventory with their reasons. NOT YET VERIFIED: production serves 9b2b44c, which contains the defect. It closes when the fix is deployed, not before.`
...
- **Fix SHA**: `cc94f5bb63a418ae64314f641e657ec69e11e336`
- **Verification evidence**: `RLS_BYPASS_INVENTORY.md is now generated by scripts/certification/render-rls-bypass-inventory.mjs from app/, lib/ and workers/, so completeness is a property of the scan rather than a promise. The reason each site is safe lives in rls-bypass-rationales.json, reviewed against this branch, and a site with no entry renders UNREVIEWED and exits the generator non-zero. Current state: 69 sites - 20 Category A, 8 Category B, 41 Category C - and 0 unreviewed, exit 0. Negative control run: a throwaway lib/__nc/canary.ts containing bypassRls:true took the scan to 70 sites, 1 unreviewed, exit 1, and reported drift; removing it returned 69, 0 and exit 0. Two tests in tests/rls-bypass-inventory.test.ts make both halves load-bearing in CI - an unannotated bypass fails the build, and so does hand-editing the generated document. 2 passed. The false claim is gone: the document now states that isolation is enforced by the Prisma extension alone, that DB-level RLS is built and proven but not applied, and that "any statement that this system enforces isolation at the database layer today is wrong". It also carries classification: CURRENT_CANONICAL, which it previously lacked. Scope is the request path and the document says so; including scripts/ would have produced 273 sites across 91 files and buried the ones a request can reach.`

### `TEL-P2-033` — Doctor Reported Docker Available By Reading The Client Binary, While The Daemon Could Not Build Or Even Prune

- **Severity**: P2
- **Status**: `VERIFIED`
- **Owner**: core-team
- **Discovered**: 2026-08-26T09:30:00.000Z
...
- **Fix SHA**: `N/A`
- **Verification evidence**: `The mechanism is fixed and the fix is proven; the production symptom is not yet re-measured, because only a redeploy clears it. docker-compose.build.yml now requires all three arguments with no defaults, using the ${VAR:?message} form the repository already uses for CRM_IMAGE. Controls run 2026-08-26: with the arguments absent, `docker compose -f docker-compose.build.yml config` exits 1 with "required variable APP_COMMIT is missing a value: APP_COMMIT is required. A locally built image must still name the commit it came from..."; with them supplied it exits 0 and resolves APP_COMMIT, APP_VERSION and APP_BUILT_AT for both web and worker. Defaulting APP_COMMIT to anything derived inside the overlay was deliberately not done: a plausible-looking SHA on an image nobody can trace is worse than "unknown", which lib/release.ts can at least recognise as absent. tests/build-overlay-identity.test.ts holds the line - 8 tests asserting each argument is passed, each uses the fail-closed :? form rather than a silent :- default, no commit is invented, and the Dockerfile still defaults to "unknown", which is what makes the requirement load-bearing. Mutation proof: removing the args block turned 6 of the 8 red; restoring returned 8 passed. docs/GCP_DEPLOY.md Phase 7 now states that building on the box is not how this system is deployed, cites the DEPLOY.md rule, explains that this defect is how production came to report "unknown", and shows the exports a build on the box needs. NOT YET VERIFIED: production still answers "unknown". Closing this needs a redeploy from a published digest, after which scripts/certification/record-deployed-state.mjs answers the question in one command and refuses to record a pass it did not observe. Also unanswered from here, and unanswerable without host access: which image the running container was actually started from, and therefore whether it contains the TEL-P0-013 fix.`
