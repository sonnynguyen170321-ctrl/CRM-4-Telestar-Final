...
|---|---|---|---|---|
| **P0** (Launch Blocker) | 12 | 12 | 0 | **0** |
| **P1** (Critical) | 46 | 44 | 0 | **2** |
...
- **Fix SHA**: `834ea2c`
- **Verification evidence**: `Hardcoded credentials removed from all 15 executable paths, scripts/liveCredentials.ts enforced with fail-closed checks, verified by tests/no-committed-credentials.test.ts (35 passed, exit 0).`
...
- **Fix SHA**: `64a4ec9`
- **Verification evidence**: `Closed the tenant boundary with 146 composite foreign keys across 66 models (migration 20260827000000_composite_tenant_foreign_keys). Verified by scripts/audit-tenant-integrity.mjs (0 unkeyed edges) and tests/tenant-includes.test.ts. Applied and live in production.`
...
- **Verification evidence**: `ALL SIX LINKS NOW HOLD, MEASURED. The previous record was honest that four of six held and that the remaining two - whether the running web and worker containers were started from the candidate digest - were answerable only on the deployment host, which the workstation could not reach. It can now: port 22 is open to 35.235.240.0/20, so an IAP tunnel reaches it. verify-release-identity.mjs gained --via, which takes a command prefix that opens a shell on the host; the tool appends its own docker inspect, writes the raw answer to an artifact before judging it, and records the transport used. --web and --worker are refused outright rather than deprecated: accepting a digest typed on the command line is what record-release-identity.mjs did, and is how this record came to claim sha256:99fbfe, a digest appearing nowhere else in the evidence tree. Measured 2026-08-27 for candidate 396d365, exit 0, status PASS: CI run 33108408475 headSha 396d365 conclusion success; image sha256:0b0f49774e3b64d2d77c0a682a3b2fb52ed51762596669ef4e2f21a1ffa2a9eb carrying revision label 396d365; health SHA 396d365; and both running containers observed on that digest from the host, containerDigestsObserved true. Four artifacts. The RepoDigests parser lives in scripts/certification/lib/repoDigest.mjs so it is testable without importing a module whose body probes production. tests/release-identity-digest-parsing.test.ts covers the case that actually loses releases - an image carrying digests for several repositories, where taking the first entry certifies a mirror - plus an unpushed image with no RepoDigests, a malformed digest, and a repository whose name merely prefixes the release repository. 7/7.`
...
- **Fix SHA**: `N/A`
- **Verification evidence**: `CLOSED BY PERFORMING THE THING THAT WAS MISSING. The defect was that EV-DR-ROLLBACK claimed candidate 9b2b44c while citing a transcript from a drill run a day earlier for c7bf639, and that no rollback had ever been performed for the candidate it named. A rollback has now been performed. Against production, for candidate 396d365: swap to sha256:0b0f4977, swap back to the previous release 7592278 at sha256:6cd77511, swap forward again. Each phase went through scripts/rollback.sh - the script an operator would actually run - and each was verified by reading the running containers and the health endpoint rather than by asserting the swap worked. Rollback 18.76s, restore 18.78s, findings none, DR-003 PASS, exit 0. EV-DR-ROLLBACK now carries candidateSha 396d365, startedAt 2026-08-27T19:44:16.534Z and finishedAt 19:45:42.362Z from the process clock, and three artifacts of 4861, 4867 and 4859 bytes, one per phase, each hashed. The candidate digest appears 13 times in the restore transcript, so checks U and U2 have something to corroborate against. Getting there required removing two defects that made a passing drill impossible and which each hid the other: TEL-P1-057 (rollback.sh demanded a terminal, so the drill aborted all three phases) and TEL-P1-058 (the host scripts were never executable in git, so fixing the first made the next checkout produce mode 644). Both are fixed and this drill ran from a clean checkout of the candidate. One further gap was found and fixed while closing this: buildRollbackEvidence hardcoded artifacts: [], so the drill wrote a transcript per phase and cited none of them - the same shape as this defect. The record now cites all three.`
...
- **Fix SHA**: `N/A`
- **Verification evidence**: `REOPENED — CLOSED PREMATURELY, AND THE VALIDATOR CAUGHT IT. This defect was marked VERIFIED on 2026-08-27 on the strength of EV-BRANCH-PROTECTION and EV-DR-ROLLBACK being regenerated by tools that ran. That reasoning checked only the two records with whole-MINUTE bounds and read the defect as if those were all it named. It names seven, and the other three carry whole-SECOND, zero-millisecond bounds, which its own text spells out: EV-EMAIL-EXACTLY-ONCE at 21:50:57.000Z, EV-ROLE-MODEL at 21:51:20.000Z, EV-SECURITY-BOUNDARIES at 21:51:38.000Z. The closure claimed "no record in the evidence tree now carries a whole-minute, zero-millisecond pair", which was true and was not the property that mattered. certify:validate reported all three on the next run, under check A (evidence for non-candidate SHA 9b2b44c) and check TIME_IMPOSSIBILITY (execution predates the candidate freeze). A record that no tool produces cannot be regenerated for a new candidate, so each re-freeze strands it further behind - which is the shape of the defect rather than a side effect of it. RESOLVED SO FAR (4 of 7), each by a tool that ran and wrote its own timestamps from the process clock: EV-DEPLOYED-STATE by record-deployed-state.mjs, EV-RELEASE-IDENTITY by verify-release-identity.mjs, EV-RLS-POSTURE by collect-rls-posture.mjs, EV-BRANCH-PROTECTION by record-branch-protection.mjs, and EV-DR-ROLLBACK by dr-rollback-drill.mjs performing an actual rollback. REMAINING (3 of 7): EV-EMAIL-EXACTLY-ONCE, EV-ROLE-MODEL and EV-SECURITY-BOUNDARIES. Each names a real command and cites a raw artifact, and the suites they describe do pass - they are re-run every ladder run inside gates 11, 14 and 16. What is missing is a producer: nothing runs those specific command sets and writes the record, so the record is authored rather than measured, and its metrics (test counts, roles exercised, gitleaks commits scanned) are transcribed rather than parsed. Closing this needs a collector that runs each command set, parses the counts out of the run, hashes the artifact it wrote, and stamps the process clock.`
...
- **Verification evidence**: `Fixed in commits 927abc9 and ab1496d, verified by tests/ai-route-cross-tenant-red-team.test.ts, and deployed to production at commit 28676f1.`
