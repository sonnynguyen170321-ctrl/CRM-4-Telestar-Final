...
- **Fix SHA**: `N/A`
- **Verification evidence**: `CLOSED. Seven records carried whole-minute, zero-millisecond start/finish pairs. Five were regenerated earlier by tools that ran. The two that remained are now regenerated as well, and neither could have been until the work behind it was actually done. EV-BRANCH-PROTECTION: produced by scripts/certification/record-branch-protection.mjs at 2026-08-27T19:46:36.838Z -> 19:46:37.736Z, millisecond precision from the process clock. That tool was itself defective when first written and is recorded as TEL-P1-056: it defaulted every control with ?? true and stamped three behavioural claims on as constants, then overwrote the 2790-byte behavioural transcript it cited with a 295-byte API readout. It now reads every control with no defaults, fails closed when one is absent, writes its API readout to a separate artifact, and cites the restored transcript with behaviouralEnforcement { measuredOn: 2026-08-25, measuredByThisRun: false } - which says in the record that this run did not perform the probe. EV-DR-ROLLBACK: could not be regenerated at all until a rollback was performed. One was, on 2026-08-27 for candidate 396d365, and the record now carries 19:44:16.534Z -> 19:45:42.362Z with three hashed artifacts. See TEL-P0-012. No record in the evidence tree now carries a whole-minute, zero-millisecond pair. Validator check V is what enforces this going forward.`
